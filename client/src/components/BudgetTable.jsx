...
// (STEP4는 원래 저장 대상이 아니므로, 이 선택 상태는 서버로 전송하지 않는 순수 탐색용이다)
...
const TOTAL_REQUEST = ITEMS.reduce((sum, it) => sum + it.amount, 0); // 58
const BUDGET = 50;
const SHORTFALL = TOTAL_REQUEST - BUDGET; // 8

export default function BudgetTable() {
  const [given, setGiven] = useState({}); // { 항목명: true }

  const toggle = (name) => {
    setGiven((prev) => ({ ...prev, [name]: !prev[name] }));
  };

  const givenAmount = ITEMS.filter((it) => given[it.name]).reduce((sum, it) => sum + it.amount, 0);
  const enoughGivenUp = givenAmount >= SHORTFALL;
    ...
    <div style={{ margin: '14px 0 4px' }}>
      ...
      <div style={{ marginTop: 10, paddingTop: 10 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
          ...
          <span style={{ fontWeight: 600, color: 'var(--color-navy)' }}>{TOTAL_REQUEST}억</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 4 }}>
          <span style={{ color: 'var(--color-text-body)' }}>실제 예산</span>
          <span style={{ fontWeight: 600, color: 'var(--color-navy)' }}>{BUDGET}억</span>
        </div>
        ...
        >
          <p style={{ margin: 0, color: '#FFFFFF', fontSize: 13, fontWeight: 500 }}>
            {enoughGivenUp
              ? `예산 안에서 집행할 수 있습니다. (${givenAmount}억 확보 / ${SHORTFALL}억 필요)`
              : `예산이 초과되어 정책 집행이 어렵습니다. (${givenAmount}억 확보 / ${SHORTFALL}억 필요)`}
          </p>
        </div>
      </div>
